...
class SizesControllerTest < ActionController::TestCase
  ...
  test "should create size" do
    assert_difference('Size.count') do
      post :create, size: { product_id: @size.product_id, shape_id: @size.shape_id, stock: @size.stock }
    end
  ...
  test "should update size" do
    patch :update, id: @size, size: { product_id: @size.product_id, shape_id: @size.shape_id, stock: @size.stock }
    assert_redirected_to size_path(assigns(:size))
  end
